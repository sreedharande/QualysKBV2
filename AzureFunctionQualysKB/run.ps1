<#
    Title:          Qualys KnowledgeBase (KB) Data Connector V2
    Language:       PowerShell
    Version:        1.0
    ...
    if ((($body |  Convertto-json -depth 20).Length) -gt 25MB) {        
        ...
        foreach ($record in $body) {            
            ...
            if ($tempDataSize -gt 25MB) {                
                $response = Invoke-WebRequest -Body $tempdata -Uri $logAnalyticsUri -Method $method -ContentType $contentType -Headers $headers -UseBasicParsing
